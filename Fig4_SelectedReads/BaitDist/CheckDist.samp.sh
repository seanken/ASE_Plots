...
shuf -n 100000 temp.sam > temp.samp.sam

cat header.txt temp.samp.sam | samtools view -Sb > temp.bam
#rm temp.sam
...
echo Get closest
bedtools closest -a read.sort.bed -b baits.sort.bed -d -t first > close.samp.bed
